...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("email = lower(email) AND length(email) BETWEEN 3 AND 320", name="ck_users_email_normalized"),
        sa.CheckConstraint("length(display_name) BETWEEN 1 AND 80", name="ck_users_display_name_length"),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], name="fk_credentials_user_id_users", ondelete="CASCADE"),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "revoked_reason IS NULL OR revoked_reason IN ('signed_out','refreshed','reuse_detected','expired')",
        ...
        sa.UniqueConstraint("access_token_fingerprint", name="ux_sessions_access_token_fingerprint"),
    )
    op.create_index("ix_sessions_user_id", "sessions", ["user_id"])
    op.create_index("ix_sessions_family_id_active", "sessions", ["family_id"], postgresql_where=sa.text("revoked_at IS NULL"))
    op.create_table(
        "refresh_tokens",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.ForeignKeyConstraint(["session_id"], ["sessions.id"], name="fk_refresh_tokens_session_id_sessions", ondelete="CASCADE"),
    ...
    op.create_index("ix_refresh_tokens_family_id_active", "refresh_tokens", ["family_id"], postgresql_where=sa.text("revoked_at IS NULL"))

...
class TestListMyEntries:
    ...
    async def test_returns_entries_in_desc_order(self) -> None:
        user = uuid4()
        e1 = _entry(user, datetime(2026, 1, 1, tzinfo=UTC))
        e2 = _entry(user, datetime(2026, 1, 3, tzinfo=UTC))
        e3 = _entry(user, datetime(2026, 1, 2, tzinfo=UTC))
    ...
    async def test_other_user_entries_not_returned(self) -> None:
        user_a = uuid4()
        user_b = uuid4()
        e_a = _entry(user_a, datetime(2026, 1, 1, tzinfo=UTC))
        e_b = _entry(user_b, datetime(2026, 1, 1, tzinfo=UTC))

...
description: "The first secure end-to-end slice: domain access, sign-in, protected routes, and a bootstrapped local user."
---

`auth-bootstrap` is the first deployable feature of Campfire. It establishes the minimum secure foundation that every later product feature can rely on: a public domain, a managed identity flow, a protected authenticated area, and an internal user record bootstrapped on first login.

## What this slice delivers

- A public landing page reachable on the Campfire domain over TLS.
- A managed sign-in and sign-out flow through Cognito Hosted UI.
- Protected routes that block unauthenticated visitors and redirect them into auth.
- A backend `/me` endpoint that validates the bearer token and returns a Campfire user-context view.
- Automatic creation of a minimum local user record on the first successful login, with idempotent reuse on every subsequent login.

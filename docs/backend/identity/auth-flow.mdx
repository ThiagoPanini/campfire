...
are summarized in [API endpoints](/backend/identity/api-endpoints), and generated
schema details live in [Backend contracts](/backend/identity/contracts).

## Register

Creates a user and credentials row. Registration does not return tokens; the
frontend logs in immediately after successful creation.
...
sequenceDiagram
  ...
  API->>DB: insert user and credentials
...
| Related | [API endpoints](/backend/identity/api-endpoints), [Authorization](/glossary/authorization) |
...
| Related | [Backend contracts](/backend/identity/contracts), [OpenAPI](/glossary/openapi) |
...
| Related | [Identity context](/backend/identity/overview), [Testing strategy](/development/testing-strategy) |

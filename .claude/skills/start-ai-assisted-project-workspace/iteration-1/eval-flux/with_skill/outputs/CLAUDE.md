...
`flux` is a Pomodoro timer that logs what you worked on each focused session and produces a heatmap comparing where your time actually went versus where you thought it went. Full vision in [PRODUCT_VISION.md](./PRODUCT_VISION.md). Current MVP scope in [docs/mvp-scope.md](./docs/mvp-scope.md).
...
Each app under `apps/` is expected to expose a `dev/run_local.py` (or framework-equivalent) entrypoint for end-to-end debugging through the IDE debugger. The corresponding `.vscode/launch.json` configuration is committed to the repo.

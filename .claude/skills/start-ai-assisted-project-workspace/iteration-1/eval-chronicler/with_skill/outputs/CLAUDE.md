...
`campaign-chronicler` is a tool for D&D groups to log each player's per-session character actions and generate a chronicle-style lore document at the end of the campaign. Full vision in [PRODUCT_VISION.md](./PRODUCT_VISION.md). Current MVP scope in [docs/mvp-scope.md](./docs/mvp-scope.md).

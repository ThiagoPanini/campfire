...
**Created**: 2026-04-29
**Status**: Draft
**Input**: User description: "Complete CI/CD Pipeline for Campfire — automate validation and Render deployments with a Spec-Kit-aligned branch strategy (feature → develop → main), separate develop/production environment controls, post-deployment health checks, and operational documentation."

## Clarifications

### Session 2026-04-29

- Q: What live database migration strategy should the CI/CD pipeline use for Render environments? → A: Develop free API runs migrations in its Render start command; production uses Render Pre-Deploy Command.
- Q: What should happen if a free develop database is used temporarily? → A: Treat develop data as disposable, then migrate and seed required test data after recreation.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Validated pull request into `develop` (Priority: P1)
...
the `develop` branch, the platform must run a complete validation pipeline
...
the pipeline. Without trustworthy validation on `develop` PRs, automated
...
`develop` even before any deploy automation is wired up.

**Independent Test**: Open a draft PR from a feature branch into
`develop` containing a deliberate failure (e.g. a TypeScript type error or
...
1. **Given** a feature branch following the `###-feature-name` pattern, **When** a pull request targeting `develop` is opened or updated, **Then** the CI pipeline runs the full validation suite and reports each job as a separate, named check.
...
### User Story 1A - Automatic feature branch PR into `develop` (Priority: P2)
...
`develop`, provided the branch follows the `###-feature-name` convention
...
that do not match the feature pattern, have no diff against `develop`, or
...
small documentation-only change. Verify that a PR targeting `develop` is
...
1. **Given** a pushed branch that matches `###-feature-name` and is not `main` or `develop`, **When** the auto-PR workflow runs, **Then** it creates a pull request targeting `develop` if no open PR exists for that head/base pair.
2. **Given** an open PR already exists from the feature branch into `develop`, **When** the branch receives another push, **Then** the workflow refreshes PR metadata/body/labels in place and does not create a duplicate PR.
...
4. **Given** the feature branch has no diff against `develop`, **When** the workflow runs, **Then** it exits successfully without opening a PR.

---

### User Story 2 - Automatic develop deployment on merge into `develop` (Priority: P1)

When a pull request is merged into `develop`, the platform must
automatically deploy the merged commit to the Render develop environment
(backend API + frontend web), without any manual step. After the deploy
hooks complete, the workflow must validate that the develop API responds
on `/healthz` and `/readyz` and that the develop frontend URL responds
...
**Independent Test**: Merge a trivial change into `develop`. Verify that
(a) the deploy workflow triggers automatically, (b) Render develop
...
1. **Given** a successful CI run on `develop`, **When** the commit lands on the `develop` branch tip, **Then** the develop CD workflow triggers exactly once for that commit.
2. **Given** Render returns a non-success response from a deploy hook, **When** the develop CD workflow runs, **Then** the workflow fails and the failing service name and environment are visible in the job summary.
3. **Given** the develop deploy hooks succeed, **When** post-deploy validation runs, **Then** the workflow probes `/healthz`, `/readyz`, and the frontend public URL with bounded retries and fails the run if any probe does not return success within the timeout.
4. **Given** two merges land on `develop` in quick succession, **When** the CD workflow runs, **Then** concurrency control ensures only one deploy executes at a time per environment, and the older run is cancelled or queued in a defined way.

---

### User Story 3 - Automatic `develop → main` promotion PR (Priority: P2)

After a develop deployment succeeds, the platform must automatically
create a pull request from `develop` into `main` (or update the existing
...
(post-deploy probes green on develop, migrations applied, secrets
present).

**Why this priority**: This guarantees that `main` is never silently out
of date relative to `develop`. It removes the cognitive load of
remembering to open the promotion PR and gives the maintainer a single
place to gate production releases.

**Independent Test**: Trigger a successful develop deployment. Verify
that a pull request from `develop` into `main` exists with an up-to-date
title, description, and commit list. Trigger a second successful develop
deploy and verify the same PR is updated, not duplicated.

**Acceptance Scenarios**:

1. **Given** a develop deployment succeeds with all post-deploy probes green, **When** the workflow finishes, **Then** a pull request from `develop` into `main` is created if none is open, or updated in place if one is already open.
2. **Given** the develop deployment fails or has red probes, **When** the workflow finishes, **Then** no `develop → main` promotion PR is created or updated.
...
When a pull request from `develop` into `main` is approved by a human
...
develop. Without this gate, a misconfigured workflow could ship untested
code to real users; without separated secrets, a leaked develop secret
...
satisfied, using production secrets distinct from develop secrets, and
that probes run against production URLs.

**Acceptance Scenarios**:

1. **Given** a `develop → main` pull request is approved and merged, **When** the production CD workflow runs, **Then** it requests approval through the protected GitHub Environment before any deploy hook is called.
...
The maintainer must be able to redeploy a known-good commit to develop or
...
**Independent Test**: From the GitHub Actions UI, dispatch the develop CD
workflow against the `develop` branch and confirm a redeploy + probes
run. Dispatch the production CD workflow against `main` and confirm the
GitHub Environment approval gate is enforced.

**Acceptance Scenarios**:

1. **Given** the maintainer dispatches the develop CD workflow on the `develop` branch, **When** the run starts, **Then** it deploys develop and runs the standard post-deploy probes.
...
configure Render services and deploy hooks for develop and production,
(c) how to configure branch protection on `develop` and `main`, (d) the
...
1. **Given** a new contributor reads the operations documentation, **When** they configure GitHub Actions, secrets, environments, branch protection, and Render hooks, **Then** the develop pipeline runs successfully end-to-end without further guidance.
...
- A pull request targets `main` directly from a feature branch (bypassing `develop`): branch protection on `main` must reject the merge unless the source branch is `develop`.
- A force-push to `develop` or `main`: branch protection must disallow force-pushes on both branches.
...
- A temporary free develop Render PostgreSQL database expires or is recreated: the runbook must treat develop data as disposable and restore only through migrations plus required seed/test data.
...
- Two merges into `develop` happen within seconds of each other: concurrency control ensures only one deploy is in flight per environment.
...
- The promotion PR (`develop → main`) is closed manually by the maintainer: the workflow detects the closed state on the next develop deploy and creates a fresh PR rather than reopening the closed one.
...
- **FR-001**: The system MUST treat `main` as the production branch and `develop` as the integration branch, with all feature work landing first on `develop` via pull request from a `###-feature-name` branch.
- **FR-002**: Branch protection on `develop` MUST require all designated CI checks to pass before merge and MUST disallow force-pushes and direct pushes.
...
- **FR-005**: The system MUST automatically create or update a pull request from a pushed `###-feature-name` branch into `develop` when the branch has a diff against `develop` and no valid open PR already exists.
- **FR-006**: The feature-to-develop PR automation MUST be idempotent: repeated pushes to the same feature branch MUST update the existing PR metadata/body/labels and MUST NOT create duplicates.
- **FR-007**: The feature-to-develop PR automation MUST ignore `main`, `develop`, tags, deleted branches, branches without a diff against `develop`, and branches that do not match the `###-feature-name` pattern.
- **FR-008**: The feature-to-develop PR automation MUST use only repository-scoped GitHub credentials with the minimum required permissions (`contents: read`, `pull-requests: write`) and MUST NOT push commits, rewrite history, or modify application files.
- **FR-009**: CI MUST include a policy check for pull requests targeting `main` that fails unless the source branch is `develop`, because repository settings may not natively enforce source-branch restrictions across all GitHub plans.

#### CI pipeline (validation, runs on PRs into `develop` and on pushes to `develop`/`main`)
...
- **FR-030**: The develop CD workflow MUST trigger automatically only after the CI workflow for the `develop` branch tip has completed successfully, and MUST also be invokable via `workflow_dispatch` on the `develop` branch.
- **FR-031**: The develop CD workflow MUST NOT run if the corresponding CI run for that commit did not succeed.
- **FR-032**: The develop CD workflow MUST call the configured Render deploy hook(s) for the develop backend and the develop frontend.
- **FR-033**: The develop CD workflow MUST fail if any deploy hook returns a non-success response, and MUST identify which service/environment failed in the job summary.
- **FR-034**: After deploy hooks succeed, the develop CD workflow MUST probe the develop API `/healthz` and `/readyz` endpoints and the develop frontend public URL with bounded retries and a defined timeout, and MUST fail if any probe does not return success.
- **FR-035**: The develop CD workflow MUST use a concurrency group keyed on the develop environment so that only one develop deploy runs at a time.
- **FR-036**: After a successful develop deployment, the workflow MUST create or update a pull request from `develop` into `main`, with a description summarizing the commits being promoted and a production-readiness checklist.
...
- **FR-043**: The production CD workflow MUST use production-scoped secrets and variables, distinct from develop, and MUST NOT echo or print secret values in logs.
...
- **FR-050**: The repository MUST include operational documentation, reachable from a single entry point, that covers: GitHub Secrets configuration per environment, GitHub Environments and protection rules, branch protection rules for `develop` and `main`, Render service and deploy hook configuration, Render service environment variables, the migration execution strategy per environment, the complete inventory of expected secrets and variables (per environment, marked develop-ready vs production-pending), the rollback procedure, and a troubleshooting guide.
- **FR-050A**: While the develop API service uses the free tier without Render pre-deploy commands, live develop database migrations MUST run from the Render API start command before `uvicorn`; the documentation MUST explain this trade-off and the paid-plan path to a real Render Pre-Deploy Command.
- **FR-050B**: The develop PostgreSQL database SHOULD use a persistent paid Render Postgres plan. If a free database is used temporarily, the runbook MUST treat develop data as disposable and recover through migrations plus required seed/test data.
...
- **CI Pipeline**: The set of validation jobs run on pull requests into `develop` and on pushes to `develop`/`main`. Inputs: the commit under test. Outputs: pass/fail per job, build artifacts where useful.
- **Feature PR Automation**: The workflow that observes pushes to `###-feature-name` branches and creates, updates, or intentionally ignores a pull request into `develop`.
- **Develop CD Pipeline**: The deployment workflow that runs on the `develop` branch tip after CI succeeds, calls Render develop deploy hooks, runs post-deploy probes, and opens/updates the promotion PR.
...
- **GitHub Environment**: A logical grouping of secrets, variables, and protection rules in GitHub Actions; one per deployment target (`develop`, `production`). Production has stricter protection (required reviewers, optional wait timer).
...
- **Branch Promotion**: The flow `feature → develop → main`. Promotion from develop to main is mediated by the auto-managed promotion PR and human approval.
- **Release Candidate**: The commit at the tip of `develop` after a successful develop deploy and green probes. It is the candidate that the promotion PR offers for production.
...
- **Secrets Inventory**: The single, authoritative list (in operational documentation) of every secret and variable consumed by the workflows, with name, scope (develop or production), and purpose.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: 100% of pull requests targeting `develop` run the full required validation suite, and 0% can merge with any required check red.
- **SC-001A**: 100% of pushed branches matching `###-feature-name` with a diff against `develop` result in exactly one open or updated pull request into `develop`; 0 duplicate pull requests are created for repeated pushes to the same branch.
- **SC-002**: 100% of merges into `develop` trigger a develop deployment automatically; 0% require manual intervention to start the deploy.
- **SC-003**: 100% of successful develop deployments result in a `develop → main` pull request that is created or updated; 0% of failed develop deployments do so.
...
- **SC-007**: 100% of develop and production deployments are followed by post-deploy probes against `/healthz`, `/readyz`, and the public frontend URL, and any unhealthy probe fails the run.
...
- **SC-009**: A maintainer (or AI agent) configuring the project from scratch can bring the develop pipeline to green by following only the operational documentation, without reading workflow source.
...
- **SC-013**: 0% of pull requests from a branch other than `develop` into `main` can pass the required CI policy gate.
...
- Automatic feature-to-develop PR creation is a convenience layer, not an approval bypass. The maintainer still reviews and merges into `develop`.
...
- The promotion PR from `develop` to `main` is sufficient as a release record for an MVP; a dedicated changelog or release-notes generator is out of scope for this feature.
...
- CI validation of Alembic against an ephemeral database does not by itself apply migrations to live Render databases. Develop live migrations are run by the Render API start command while the API remains on the free web service plan; production uses Render Pre-Deploy Command.
- The develop Render PostgreSQL database should be persistent. If a free database is used temporarily, develop data is disposable and the documented recovery path is migrations plus required seed/test data rather than backup and restore.

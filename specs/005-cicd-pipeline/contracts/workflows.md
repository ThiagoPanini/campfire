...
| `create-feature-pr` | Create or update feature PR | 10 | Runs after green `ci-status` only on feature-branch pushes; creates/updates one PR into `develop`. |
| `deploy-develop` | Deploy and probe develop | 45 | Runs after green `ci-status` only on pushes to `develop`; triggers Render hooks and probes Develop URLs. |
| `promotion-pr` | Promote develop to main PR | 10 | Runs only after `deploy-develop` succeeds; creates/updates one PR from `develop` into `main`. |
| `deploy-production` | Deploy and probe production | 45 | Runs after green `ci-status` only on pushes to `main`; gated by the `production` Environment. |
| `production-summary` | Production deployment summary | 5 | Runs for `main` push attempts and records CI/deploy results. |

### Required exit-status semantics
- `ci-status` MUST succeed iff every validation job listed before it succeeded.
- Cancelled or skipped validation dependencies count as failure for `ci-status`.
- Post-`ci-status` automation jobs do not affect the branch-protection check, but they do affect the workflow run result.
...
## 2. `deploy-develop.yml` (manual redeploy)
...
|---|---|---|---|
| `branch-guard` | Refuse non-develop refs or manual dispatch without green `CI status (aggregate)`. | `GITHUB_TOKEN` | Exits 1 with `::error::` message; no further jobs run. |
...
## 3. `deploy-production.yml` (manual redeploy)
...
| Job ID | Purpose | Inputs (env) | Failure semantics |
|---|---|---|---|
| `branch-guard` | Refuse non-main refs or manual dispatch without green `CI status (aggregate)`. | `GITHUB_TOKEN` | Exits 1; clear message per FR-053. |

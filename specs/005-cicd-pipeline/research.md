...
- **Decision**: Use Render **Deploy Hooks** (per-service HTTPS POST URLs stored as environment secrets) for both develop and production.
...
- **Decision**: `feature → develop → main`. Feature branches PR into `develop`; merging `develop` deploys develop; success opens/updates a single `develop → main` PR; merging that PR deploys production. Branch protection enforces the topology (FR-001..FR-004).
...
- **Decision**: Two environments — `develop` and `production`.
  - `develop`: deployment-branches restricted to `develop` only; secrets scoped to develop Render services and URLs; **no required reviewers** (auto-deploy on merge).
...
- **Alternatives considered**:
  - *Single environment with conditional logic*: rejected — would expose production secrets to develop jobs and lose GitHub's native branch restriction.

## 5. Concurrency policy

- **Decision**:
  - `ci.yml`: `concurrency: { group: ci-${{ github.ref }}, cancel-in-progress: true }` — cancel obsolete PR runs on push.
  - `deploy-develop.yml`: `concurrency: { group: deploy-develop, cancel-in-progress: false }` — queue, never cancel a deploy mid-flight.
...
- **Alternatives considered**: *Cancel on develop too*: rejected — risks stranding a half-applied migration if a second merge cancels the first deploy mid-Alembic.
...
- **Alternatives considered**:
  ...
  - *`gh` + a separate "release-candidate" branch*: rejected — extra branch to maintain; the action targets `develop` directly.
...
- **Alternatives considered**: *All values as secrets*: rejected — masking the public URL of develop in logs makes failure triage harder for a solo maintainer.
...
- **Decision**: Migrations run **in CI** (the `migrations-contracts` job applies them to an ephemeral DB). Production uses Render **Pre-Deploy Command** (`uv run alembic upgrade head`) because the production API is on a paid plan. Develop keeps the API on Render's free web service plan, so the Develop API start command runs `uv run alembic upgrade head && uv run uvicorn ...` instead of using `preDeployCommand`.
- **Rationale**: Render Pre-Deploy Commands are paid-only. Running Develop migrations in the Render start command keeps database access inside Render, avoids exposing a live database URL to GitHub Actions, and is acceptable for a single-instance Develop service where failed migrations should fail the deploy before `/readyz` passes.
...
- **Decision**: Documented in `docs/backend/ops/cicd.mdx` (not enforced by YAML — branch protection is a repo setting). Required state:
  - `develop`: require PR; require `ci-status`; block force-pushes; block direct pushes; do **not** require approval (solo maintainer + AI).
  - `main`: require PR; require ≥1 approval; require `ci-status`; require branch up-to-date; restrict source branch to `develop`; block force-pushes; block direct pushes.
...
- Move Develop migrations to Render Pre-Deploy Command if the Develop API is upgraded from the free plan.

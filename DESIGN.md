...
Design intent and visual rules for campfire's interface. Product strategy (problem, users, roadmap) lives in [PRODUCT_VISION.md](./PRODUCT_VISION.md); this document is the canonical source for *how the product should feel and look*, across both brand and product surfaces.

## 1. Brand personality

**Three words:** warm, nostalgic, intimate.

**Voice:** lowercase, spare, written like a friend handing you a mixtape — not like a brand handing you onboarding copy. No exclamation marks. No emoji. No growth-hack idioms ("level up", "supercharge", "unlock"). Honors silence; says less than it could. The UI ships in Brazilian Portuguese per [ADR 0005](./docs/decisions/0005-ui-language.md); the lowercase, intimate posture is invariant across languages.
...
Five strategic principles, derived from the personality and anti-references below. Read these before making any structural design call.
...
What campfire must not become, in order of threat. The "Don't" rules in [§7](#7-dos-and-donts) are the tactical version of this list.
...
- **Public social networks (Instagram, TikTok music, Twitter/X).** Infinite feeds, exposed engagement metrics, creator-economy copy, vertical video formats, public profiles, global discovery. campfire is sealed: interactions live inside the friend graph and never escape it.
- **Education / gamified practice apps (Yousician, Simply Piano, Fender Play).** Streak counters, badges, level meters, "next lesson" CTAs, paternal tutor voice. campfire does not teach — it logs. No progress bars on songs.
...
- **Generic SaaS-dashboard look (Notion/Linear/Vercel-style).** Cream-tinted neutrals, sidebar + topbar, identical card grids, Lucide icons everywhere. The lo-fi commitment excludes this by construction, but watch for it creeping in via component-library defaults (shadcn/ui, MUI, Chakra).
...
- Small-text contrast ≥4.5:1; large-text contrast ≥3:1. Watch the muted ink token (`--ink-muted` at 64% opacity) — confirm it passes against the warm-dark canvas at every type size it appears.
- Visible focus rings on every interactive element. Ghost links use `outline: 1px solid var(--ink); outline-offset: 7px`; primary/outline buttons use cyan with `outline-offset: 4px`.
...
- `prefers-reduced-motion: reduce` honored — video freezes, scanline/grain/tracking animations stop. Implemented in [apps/web/src/home/Home.css](apps/web/src/home/Home.css); apply the same rule to any new motion.
...
The components below already exist on the `mvp/lofi-style` branch and are the load-bearing reference implementations of this system. New surfaces should match their patterns before introducing new ones.

- **Home** ([apps/web/src/home/](apps/web/src/home/)) — the full-bleed video poster, the canonical Hero Band recipe. The CTAs (`entrar`, `criar conta`) link to `/signin` and `/signup`.
- **Nav** ([apps/web/src/ui/Nav.tsx](apps/web/src/ui/Nav.tsx), [Brand.tsx](apps/web/src/ui/Brand.tsx)) — the brand-left, action-right pattern from [§8 Navigation](#navigation).
- **Modal** ([apps/web/src/ui/Modal.tsx](apps/web/src/ui/Modal.tsx)) — overlay surface used by `/signup`, `/signup/confirm`, `/signin`. Each modal renders **on top of `Home`** so the poster stays visible behind the form. `ModalBadge` is the small label that opens each modal.
- **Field** ([apps/web/src/ui/Field.tsx](apps/web/src/ui/Field.tsx)) — the reference for [§8 Inputs / Fields](#inputs--fields) (hairline bottom border, label above, helper/error below).
- **Button** ([apps/web/src/ui/Button.tsx](apps/web/src/ui/Button.tsx)) — primary / outline / ghost variants with `isLoading` + `loadingLabel` support.
- **CodeBoxes** ([apps/web/src/ui/CodeBoxes.tsx](apps/web/src/ui/CodeBoxes.tsx)) — six-cell OTP input on `/signup/confirm`, with paste-anywhere behaviour, shake-on-error, and `aria-label` on each digit.
- **StrengthMeter** ([apps/web/src/ui/StrengthMeter.tsx](apps/web/src/ui/StrengthMeter.tsx)) — 16-segment password meter under the password field on `/signup`.
- **GhostLink, FooterHairline, PageColumn, AppPlaceholder** ([apps/web/src/ui/](apps/web/src/ui/), [apps/web/src/app/Placeholder.tsx](apps/web/src/app/Placeholder.tsx)) — small primitives used by the post-signin surface (`/app`) and the auth flows.

Design tokens are duplicated in two places today: [apps/web/src/ui/tokens.css](apps/web/src/ui/tokens.css) (the canonical CSS variables for new surfaces) and the `--vhs-red` / `--vhs-cyan` / `--bg` locals scoped to [apps/web/src/home/Home.css](apps/web/src/home/Home.css). Prefer the `tokens.css` names (`--tracking-red`, `--crt-chroma`, `--bg`) when adding new components; collapse the Home-local copies on the next pass through `Home.css`.

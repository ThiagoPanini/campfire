...
5. **Transactions**: opened/closed by the request-scope adapter (`get_db_session` → `session_scope` in `shared/persistence/session.py`, which commits on success and rolls back on exception). Neither use cases nor repositories may call `session.commit()` — repos call `session.flush()` only. (Constitution §"Backend Architecture Invariants" #5 phrases this as "the use case must not commit"; in practice this also covers repos, since two repos committing inside one HTTP request defeats unit-of-work. `email_confirmation_repository.update()/invalidate_pending_for()` currently violate this — tracked as AUDIT.md Q-1; don't pattern-match off it for new repos.)
